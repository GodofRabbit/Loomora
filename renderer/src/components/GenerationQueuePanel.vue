...
const emit = defineEmits(['toggle-pause', 'retry', 'remove', 'clear-finished']);
  ...
  <Transition name="generation-queue">
    <aside v-if="open" class="generation-queue-panel" aria-label="生成队列">
      <header>
        ...
        <button type="button" title="关闭队列" @click="open = false">
          <X aria-hidden="true" />
        </button>
      </header>
      ...
      <div class="generation-queue-list">
        <article v-for="task in tasks" :key="task.id" :class="task.status">
          <div class="generation-queue-item-head">
            <span>{{ statusText[task.status] || task.status }}</span>
            <small>{{ taskTime(task.createdAt) }}</small>
          </div>
          <p>{{ task.prompt }}</p>
          <small>
