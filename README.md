...
Loomora 是一款基于 Electron、Vue 3 和 Vite 构建的跨平台 AI 图片桌面应用。它将 GPT Image 2 创作、聊天式历史、本地作品库、灵感复用、图片编辑和离线 OCR 集中在同一个紫色工作台中，并支持配置兼容 OpenAI 协议的服务地址。作品、提示词、编辑版本和整理信息均保存在本机，适合需要长期积累个人素材的创作者。
...
| 快速创作                                                   | 本地作品库                                               |
| ---------------------------------------------------------- | -------------------------------------------------------- |
| ![快速创作工作台](docs/screenshots/creation-workspace.jpg) | ![本地作品库](docs/screenshots/works-gallery-female.jpg) |

| 灵感广场                                             | 图片编辑与 OCR                                           |
| ---------------------------------------------------- | -------------------------------------------------------- |
| ![灵感广场](docs/screenshots/inspiration-square.jpg) | ![图片编辑与文字识别](docs/screenshots/image-editor.jpg) |
...
| 作品整理   | 收藏、回收站、专辑、标签、颜色标记、图片信息、提示词与备注编辑   |
| 数据管理   | 本地备份与恢复、导入去重、图片版本对比与恢复、可配置快捷键       |
| 服务扩展   | 多服务配置档案、Provider 适配器、按服务隔离 API Key 与生成队列   |
...
- 可直接查看图片关联的完整提示词、生成参数、尺寸、文件大小、颜色、标签、专辑和备注，并支持编辑这些信息。
- 支持收藏和回收站，删除的图片可以恢复；彻底删除前会再次确认，避免误操作。
- 支持批量设置专辑、标签和颜色，也可以按这些信息筛选作品。
- 支持多选导出与删除，以及导出全部或当前日期作品。
- 下载会先打开系统“另存为”窗口，由用户确认文件名和保存位置后再写入文件。
- 相同文件内容的图片会通过 SHA-256 校验跳过重复导入，避免作品库出现重复项目。
- 编辑后另存为新图会生成新的作品版本；可在版本对比中恢复旧版本，恢复结果仍会作为新版本保存。
...
- OpenAI 兼容服务默认使用 `gpt-image-2`，提示词上限为 4000 个字符，单次最多生成 10 张图片；Replicate 配置需要填写 `owner/model`，当前按单张异步任务处理。
...
- OpenAI 兼容服务的单张生成使用流式中间图预览，批量生成使用一次返回多张结果的 `n` 请求；不支持流式的服务会显示普通任务进度。
- 每轮创作会保存提示词、参数、参考图信息、进度和结果，可复制、编辑、删除或重新生成。
- 生成请求先进入本地持久化队列，支持查看任务进度、暂停、重试和清理已完成任务；接口地址或 API Key 未配置时会立即提示，不会长时间等待网络请求。
- 可保存多个生图服务配置；当前内置 OpenAI 兼容和 Replicate 适配器，服务地址、模型和 API Key 按配置档案隔离，队列任务会记录对应的服务配置。
- 启动时按需恢复最近的历史对话，滚动到边界后继续加载，避免一次渲染全部图片。
- 点击左上角 Logo 或 Loomora 文字可以回到创作首屏，同时保留尚未发送的输入内容。

## 六步新手引导

首次安装后的第一次启动会通过蒙层、高亮区域和虚线曲线箭头依次介绍：
...
引导不会使用传统居中弹窗遮挡整个流程。引导状态保存在 Electron 用户数据目录，后续启动不会重复弹出，也可以从“关于与帮助”中的“查看新手引导”随时重新打开。
...
5. 前往作品库查看提示词、收藏、导入、编辑、识别、导出和整理图片。
6. 在灵感广场搜索案例，一键复用提示词或图片素材。
7. 在设置中创建本地备份，或为常用操作配置快捷键。
...
- API Key 通过 Electron `safeStorage` 加密后保存在当前设备的用户数据目录中，不会写入图库或备份包；接口地址和本地存储目录保存在本机配置中。
- 已保存的接口地址、API Key 和本地存储目录在关闭设置后仍会保留，下次打开设置可继续修改；清除本地数据会一并清除这些配置。
- 本地备份包含作品、提示词、对话、编辑版本和整理信息，不包含 API Key。
- 图片生成请求会发送到用户配置的 OpenAI、兼容服务或 Replicate；图库管理、图片编辑和 OCR 均在本地执行。

## 生图服务扩展

Loomora 使用 Provider 适配器承接不同平台的请求格式。OpenAI、OneAPI、企业内部接口和本地代理等兼容服务可以直接在设置中新增配置；当前也内置 Replicate 适配器。接入其他非兼容平台时，只需在 `electron/providers/` 中增加适配器并注册能力，图库、历史记录和生成队列会继续复用统一结果格式。

适配器可以声明是否支持流式预览、批量生成、参考图、尺寸、质量、输出格式和取消任务。平台不支持的能力会由界面隐藏或降级为普通进度状态，避免用户误以为参数已经生效。

Replicate 配置方式：在设置中新增服务档案，服务类型选择 `Replicate`，接口地址填写 `https://api.replicate.com`，API Key 填写 Replicate Token，默认模型填写对应的 `owner/model`。Replicate 生成结果会先轮询远程任务，完成后下载到本地作品库。
...
│   ├── galleryMetadata.js          # 图片提示词、标签、专辑、版本等元数据
│   ├── galleryTrash.js             # 回收站恢复与彻底删除
│   ├── backup.js                   # 本地备份与恢复
│   ├── generation.js               # 图片生成请求与流式事件
│   ├── generationQueue.js          # 持久化生成队列
│   ├── secureCredentials.js        # API Key 的系统级加密存储
│   ├── ocr.js                      # OCR 工作窗口与任务调度
│   ├── ocr-preload.js              # OCR 工作窗口安全桥接
│   └── providers/                  # 生图平台注册表与适配器
│       ├── providerRegistry.js     # Provider 注册与能力元数据
