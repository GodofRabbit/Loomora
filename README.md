...
| 快速创作                                                   | 本地作品库                                                           |
| ---------------------------------------------------------- | -------------------------------------------------------------------- |
| ![快速创作工作台](docs/screenshots/creation-workspace.jpg) | ![作品库中的女性作品展示](docs/screenshots/works-gallery-female.jpg) |

| 灵感广场                                             | 女性作品编辑与 OCR                                               |
| ---------------------------------------------------- | ---------------------------------------------------------------- |
| ![灵感广场](docs/screenshots/inspiration-square.jpg) | ![女性作品图片编辑与文字识别](docs/screenshots/image-editor.jpg) |

作品库截图中包含女性作品卡片；编辑器截图使用拿手机的女性作品，便于直观看到大图预览、编辑和 OCR 入口的实际效果。
...
- 可保存多个生图服务配置；当前内置 OpenAI 兼容适配器，服务地址、模型和 API Key 按配置档案隔离，队列任务会记录对应的服务配置。
...
Loomora 使用 Provider 适配器承接不同平台的请求格式。OpenAI、OneAPI、企业内部接口和本地代理等兼容服务可以直接在设置中新增配置，不需要修改程序；非兼容平台只需在 `electron/providers/` 中增加适配器并注册能力，图库、历史记录和生成队列会继续复用统一结果格式。

适配器可以声明是否支持流式预览、批量生成、参考图、图片编辑和取消任务。平台不支持的能力会由界面隐藏或降级为普通进度状态，避免用户误以为参数已经生效。
