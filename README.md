...
- 创作页采用聊天式历史记录，每一次生成都会保留为一轮对话，可向上回看提示词、生成参数、进度和结果图；点击结果下方的文件夹图标可在系统文件管理器中定位图片。
...
- 历史提示词下方提供复制、编辑和删除记录按钮，删除对话记录不会删除作品库图片。
...
- 作品库采用左侧日期时间线、中间作品流和右侧操作栏的三栏布局；中间顶部的标题、搜索与状态栏会在滚动时吸附于顶部。
- 作品区按原图比例采用瀑布流排版，并优先展示最新作品。
- 可点击“导入图片”或直接拖拽图片到作品库区域导入本地图片。
- 作品库左侧提供浮动日期时间线，按“全部”或某一天筛选，日期节点会显示对应作品数量；多选、批量导出和导入操作集中在右侧栏。
- 作品库支持批量导出全部、当前日期或勾选图片，导出时会自动创建文件夹。
- 点击图片可进入大图预览，并在多图间切换。
- 右键菜单支持复制、作为参考图创作、重新生成、下载、识别文字、编辑、重命名、打开文件所在位置和删除；“重新生成”仅在作品能匹配到本地创作记录时展示，并按单张重新发起请求。
...
- 点击“用此灵感创作”会一键填入快速创作，并同步推荐画面比例与分辨率；点击“作为参考图创作”会把本地素材转换为参考图。
